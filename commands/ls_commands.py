...
class CmdLs(default_cmds.MuxCommand):
    ...
    def func(self):
        ...
        执行命令
        """
        if self.cmdstring == "ls":
            if not self.args:
                self.caller.msg("用法: ls room/exit/account")
                return
            ...
            if arg == "room":
                ...
                self.caller.msg("未知的参数。用法: ls room/exit/account")

        elif self.cmdstring == "lr":
            # 列出所有房间
            self.list_rooms()

        elif self.cmdstring == "le":
            # 列出所有出口
            self.list_exits()

        elif self.cmdstring == "la":
            # 列出所有账号
            self.list_accounts()
