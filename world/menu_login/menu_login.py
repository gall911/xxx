...
def node_enter_password(caller, raw_string, **kwargs):
    ...
    def _check_input(caller, password, **kwargs):
        ...
        address = session.address
        if new_user:
            # create a new account - bypass password validation
            from django.contrib.auth.hashers import make_password
            try:
                if len(password) < 2:
                    raise ValueError("密码长度必须至少为2个字符")
                account = _ACCOUNT(username=username)
                # 直接设置密码哈希，绕过所有验证
                ...
                # 创建角色
                character = create_object(character_typeclass, key=username)
                
                # 将角色与账号关联
                account.db._playable_characters = [character]
                account.db._last_puppet = character
                character.locks.add("puppet:id(%i) or pid(%i) or perm(Builder) or pperm(Builder)" % (account.id, account.id))
            ...
            # check password against existing account - special handling for superuser
            from django.contrib.auth import authenticate
            try:
                # 首先尝试使用Django的authenticate方法，这对超级用户更可靠
                account = authenticate(username=username, password=password)
                if account:
                    # 确保获取的是Evennia账号对象
                    account = _ACCOUNT.objects.get(username__iexact=username)
                    errors = []
                else:
                    # 如果Django authenticate失败，尝试直接验证
